...
function Profile() {
    ...
    useEffect(() => {
        ...
        // Fetch shared codes
        fetch(`http://localhost:5000/shared?email=${email}`)
            .then(res => res.json())
            .then(data => setSharedCodes(data));
        // Fetch received codes
        fetch(`http://localhost:5000/received?email=${email}`)
            .then(res => res.json())
            .then(data => setReceivedCodes(data));
    }, [isLoggedIn, nav, email]);

    // Delete shared code
    const handleDeleteShared = async (id) => {
        await fetch(`http://localhost:5000/shared/${id}`, { method: 'DELETE' });
        setSharedCodes(sharedCodes.filter(item => item._id !== id));
    };
    // Delete received code
    const handleDeleteReceived = async (id) => {
        await fetch(`http://localhost:5000/received/${id}`, { method: 'DELETE' });
